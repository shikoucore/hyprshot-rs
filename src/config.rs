...
mod tests {
    ...
    fn test_generate_hyprland_binds_with_clipboard() {
        ...
        assert!(binds.contains("bind = SUPER ALT, Print, exec, hyprshot-rs -m window --clipboard-only"));
        assert!(binds.contains("bind = SUPER SHIFT ALT, Print, exec, hyprshot-rs -m region --clipboard-only"));
        assert!(binds.contains("bind = SUPER CTRL ALT, Print, exec, hyprshot-rs -m output --clipboard-only"));
    }
